...
# Make sure the target variable is retained after feature selection
...
# Train Decision Tree for motor_UPDRS with LASSO selected features
cat("Training Decision Tree for motor_UPDRS...\n")
tryCatch({
  dt_motor <- train(motor_UPDRS ~ ., data = X_train_lasso_motor, method = "rpart", trControl = ctrl, tuneGrid = data.frame(cp = grid_cp))
  
  # Print message indicating training Decision Tree for motor_UPDRS is completed
  cat("Training Decision Tree for motor_UPDRS completed.\n")
}, error = function(e) {
  print("Error occurred during training Decision Tree for motor_UPDRS:")
  print(e)
})
...
if (exists("dt_motor")) {
  ...
  predictions_motor <- predict(dt_motor, newdata = X_test_lasso_motor)
  ...
  saveRDS(dt_motor, "lasso_decision_tree_model_motor.rds")
  cat("\nModel for motor_UPDRS saved.\n\n")
} else {
  cat("Failed to train Decision Tree model for motor_UPDRS. Please check for errors.\n")
}
...
# Train Decision Tree for total_UPDRS with LASSO selected features
cat("Training Decision Tree for total_UPDRS...\n")
tryCatch({
  dt_total <- train(total_UPDRS ~ ., data = X_train_lasso_total, method = "rpart", trControl = ctrl, tuneGrid = data.frame(cp = grid_cp))
  
  # Print message indicating training Decision Tree for total_UPDRS is completed
  cat("Training Decision Tree for total_UPDRS completed.\n")
}, error = function(e) {
  print("Error occurred during training Decision Tree for total_UPDRS:")
  print(e)
})
...
if (exists("dt_total")) {
  ...
  predictions_total <- predict(dt_total, newdata = X_test_lasso_total)
  ...
  saveRDS(dt_total, "lasso_decision_tree_model_total.rds")
  cat("\nModel for total_UPDRS saved.\n\n")
} else {
  cat("Failed to train Decision Tree model for total_UPDRS. Please check for errors\n")
}
